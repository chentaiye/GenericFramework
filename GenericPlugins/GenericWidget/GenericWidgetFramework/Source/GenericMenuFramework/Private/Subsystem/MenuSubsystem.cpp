...
namespace
{
	...
	bool CanCreateMenuCollectionClass(const UClass* MenuCollectionClass)
	{
		return MenuCollectionClass
			&& MenuCollectionClass->IsChildOf(UMenuCollection::StaticClass())
			&& !MenuCollectionClass->HasAnyClassFlags(CLASS_Abstract | CLASS_Deprecated | CLASS_NewerVersionExists);
	}
}
...
UMenuCollection* UMenuSubsystem::RegisterMenuCollection(TSubclassOf<UMenuCollection> InMenuCollectionClass, bool bBuildMenu)
{
	...
	if (!CanCreateMenuCollectionClass(InMenuCollectionClass))
	{
		UE_LOG(GenericLogUI, Warning, TEXT("RegisterMenuCollection class failed because menu collection class '%s' cannot be instantiated."), *InMenuCollectionClass->GetName());
		return nullptr;
	}
