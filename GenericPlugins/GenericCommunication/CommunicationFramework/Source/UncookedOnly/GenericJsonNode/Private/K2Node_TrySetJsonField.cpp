...
#include "K2Node_TrySetJsonField.h"
...
UK2Node_TrySetJsonField::UK2Node_TrySetJsonField()
{
	FunctionReference.SetExternalMember(GET_FUNCTION_NAME_CHECKED(UBPFunctions_Json, SetJsonField), UBPFunctions_Json::StaticClass());
}

void UK2Node_TrySetJsonField::AllocateDefaultPins()
{
	...
	const UEdGraphPin* InputValuePin = GetPinAt(PinInfo.InputValuePinIndex);
	check(InputValuePin);
...
FText UK2Node_TrySetJsonField::GetTooltipText() const
{
	return LOCTEXT("TooltipText", "Set the specified Json Field, the type is determined by the input target property.");
}

bool UK2Node_TrySetJsonField::IsNodePure() const
{
	return false;
}

void UK2Node_TrySetJsonField::GetMenuActions(FBlueprintActionDatabaseRegistrar& InActionRegistrar) const
{
	...
	if (InActionRegistrar.IsOpenForRegistration(Action))
	{
		...
		{
			UK2Node_TrySetJsonField* Node = CastChecked<UK2Node_TrySetJsonField>(NewNode);
		...
		GetDataNodeSpawner->CustomizeNodeDelegate = UBlueprintNodeSpawner::FCustomizeNodeDelegate::CreateStatic(CustomizeLambda, GET_FUNCTION_NAME_CHECKED(UBPFunctions_Json, SetJsonField));
		InActionRegistrar.AddBlueprintAction(Action, GetDataNodeSpawner);
	}
}

FText UK2Node_TrySetJsonField::GetMenuCategory() const
{
	static FText MenuCategory = LOCTEXT("MenuCategory", "Json");
	return MenuCategory;
}

void UK2Node_TrySetJsonField::NotifyInputChanged() const
{
