...
#include "SpacingHorizontalBox.generated.h"

class SSpacingHorizontalBox;
class UHorizontalBoxSlot;

/** 作为带间距水平框，负责把蓝图可编辑属性同步到底层 Slate 实例。 */
UCLASS(MinimalAPI)
class USpacingHorizontalBox : public UHorizontalBox
{
	GENERATED_BODY()
...
public:
	/** 添加ChildTo间距水平盒，并同步内部缓存、显示状态或事件绑定。 */
	UFUNCTION(BlueprintCallable, Category="Spacing Horizontal Box")
	GENERICSLATEFRAMEWORK_API UHorizontalBoxSlot* AddChildToSpacingHorizontalBox(UWidget* Content);
	...
	UFUNCTION(BlueprintPure, Category="Spacing Horizontal Box")
...
private:
	/** 保存MySpacing水平Box，供当前对象后续同步逻辑读取。 */
	TSharedPtr<SSpacingHorizontalBox> MySpacingHorizontalBox;
};
